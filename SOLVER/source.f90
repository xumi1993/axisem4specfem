...
subroutine locate_src(iel_src2,xi_src,eta_src)
   ...
   real(kind=dp)        :: s, z, mydzsrc, zsrcout, dzsrc
   ...
   ! locate source
   ! Only allow sources in the solid region, fixated to northern axis.
   dzsrc = router * 1000_dp
   have_src = .false.
   do i=1,naxel
      ielem = ax_el(i) 

      ! check if source inside this element
      call inside_element(0.0_dp,zsrc,ielem,xi_src,eta_src,s,z,in_it)
      dzsrc = hypot(s,zsrc-z)
      zsrcout = z

      ! if source is in this element, finish
      if(in_it) then
         iel_src2 = ielem 
         iel_src = ielem
         have_src = .true.
         ...
         s = mapping(xi_src,eta_src,nodes,1,ielem)
         z = mapping(xi_src,eta_src,nodes,2,ielem)
         dzsrc = hypot(s,zsrc-z)
         zsrcout = z
         !print*,z
         exit 
